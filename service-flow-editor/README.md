...
Open **Settings → Canvas → Color theme** to choose **White**, **Ocean**, **Violet**, **Amber**, **Rose**, **Slate**, or **Emerald**. Ocean blue is the default. The palette applies to the entire editor, including the sidebar, canvas, nodes, connectors, and dialogs. The selected theme is remembered in this browser and restored on the next visit. Changing a theme does not modify workspace data. Settings opens as a floating page with Canvas and Gestures sections.
...
Use **Settings → Canvas → Node appearance** to set one style for every collapsed node in every graph. Set fill and border colors, enable or hide borders, adjust border width from 0 to 12 px, and choose a corner radius from 0% to 50% of the shorter side. Black shadows default to 32% opacity, 5 px blur, and a 5 px downward offset; adjust opacity from 0% to 100%, blur and offset from 0 to 24 px, or turn shadows off. Circles remain circular. Expanded containers keep their structural styling. Selected nodes have a separate highlight even with borders disabled.

**Settings → Canvas → Node typography** controls font family (sans serif, system, serif, or monospace), default font size, color, weight, italics, and line height for every node label, including expanded headers. Labels wrap to fit using the selected font. Individual nodes can still override font size in the inspector. Colors inherit the selected theme until customized; each color can return to its theme default. **Reset node appearance** restores shape, shadow, and typography defaults; font size remains a separate canvas or node preference. Settings are saved in `workspace.json`, survive reopening, and support undo/redo. Older workspaces receive defaults for missing settings.

## Explore nested architecture

The top-bar **Expand levels** control reveals one, two, or any available number of internal levels beneath the current graph. **Expand all levels** opens every populated subgraph; **Collapse all** closes this branch. Empty subgraphs remain compact. Each operation is one undo step and leaves other branches alone. The expansion layout shares necessary clearance between parallel rows instead of adding every container's growth. Unrelated rows and columns retain their positions; collapsing restores the original coordinates.
...
Drag empty canvas with the left mouse button to draw a selection box, including blank space inside expanded containers. Move an expanded container by dragging its header. Fully enclosed nodes and flow segments intersecting the box are selected. Hold `Shift` while drawing to add to the selection, or Shift-click a node or flow to toggle it. `Ctrl/Cmd+A` selects all visible elements. Drag a selected service to move selected services as a group; selecting both a container and its children moves them only once. Delete removes the selected elements in one undoable operation, including descendants and incident flows of selected services. Flow paths follow their endpoints during group moves. Pan with right-drag, middle-drag, or `Space` + drag. Right-click without moving opens the context menu; dragging does not open it. On a Mac trackpad, use two-finger scrolling to pan and pinching to zoom.

## Saving and recovery

**Present** starts an isolated, temporary editing session after saving any real pending edits. Graph edits, expansion, reparenting, undo/redo, document notes, theme changes, and sidebar preferences remain in memory; they never write JSON, Markdown, or browser recovery drafts. **Exit & restore** discards the presentation and restores the original graph, navigation, viewport, selection, preferences, and undo history in one click. Reloading also discards temporary edits. Changing workspaces is disabled while presenting. Failed real saves must be resolved before presentation can start.

**Validate** highlights services with zero incoming flows using a rose fill and red outline, with a list of affected services. Inputs to descendants count toward their ancestors; inputs to an ancestor do not count toward its children. This diagnostic includes source / sink nodes and updates as flows change. It does not alter saved node styles. Click Validate again or Clear validation to remove it.

Select a service and click **Highlight chain**, or choose **Highlight upstream and downstream** in its context menu. Upstream services and flows use purple, downstream use blue, cycles use teal, and the selected subtree uses amber. Unrelated elements fade. Traversal follows real directed edges; ancestors carry hidden endpoints' highlights without inventing connections to their children. Clear the highlight to restore normal styling.

Use a service's **Move to graph…** context action to move it and its entire subtree into another service's graph or **Root graph**. Self/descendant destinations are excluded. The service is placed beside existing destination nodes; nested local geometry, keys, IDs, documents, and flow weights remain intact. Edges receive the correct common ancestor graph and reconnected orthogonal routes. Moves autosave and support undo/redo.
...
Mouse-wheel and two-finger trackpad scrolling pan the graph in both axes. Ctrl/Command + wheel and trackpad pinch zoom only the graph around the pointer; Safari gesture events and wheel magnification are not applied twice. On a touchscreen, one finger pans empty space or drags a node, two fingers pan and pinch around their midpoint, a tap selects, double-tap expands a node, and a stationary long press opens the context menu. Touch drags have a movement threshold to distinguish taps and holds; adding a second finger switches to navigation and waits for all fingers to lift before editing again. While the canvas has keyboard focus, Ctrl/Command + `+`, `-`, and `0` zoom in, zoom out, and reset to 100%. Browser zoom remains available outside the canvas. These conventions follow Apple's [gesture guidance](https://developer.apple.com/design/human-interface-guidelines/gestures); glass controls use translucent layers, backdrop blur, and bright edges inspired by [Liquid Glass](https://developer.apple.com/documentation/technologyoverviews/liquid-glass), with a reduced-transparency fallback.
...
| Zoom                                    | Pinch, Ctrl/Cmd + wheel, or zoom buttons          |
